...
defmodule TodoGenServer do
  ...
  def start() do
    GenServer.start(__MODULE__, nil) |> register()
  end

  defp register({:ok, pid}) do
    Process.register(pid, @name)
    {:ok, pid}
  end

  def add_entry(entry) do
    GenServer.cast(@name, {:add_entry, entry})
  end

  def entries(date) do
    GenServer.call(@name, {:entries, date})
  end

  def update_entry(id, updater_fun) do
    GenServer.cast(@name, {:update_entry, id, updater_fun})
  end

  def delete_entry(id) do
    GenServer.cast(@name, {:delete_entry, id})
  end
  ...
  def handle_cast({:add_entry, entry}, state) do
    new_state = TodoList.add_entry(state, entry)
    {:noreply, new_state}
  end

  @impl true
  def handle_cast({:update_entry, id, fun}, state) do
    new_state = TodoList.update_entry(state, id, fun)
    {:noreply, new_state}
  end

  @impl true
  def handle_cast({:delete_entry, id}, state) do
    new_state = TodoList.delete_entry(state, id)
    {:noreply, new_state}
  end
...
####---------------TEst----------------------------
#TodoGenServer.start()
#TodoGenServer.add_entry(%{date: ~D[2025-07-22], title: "Scrivere GenServer"})
#TodoGenServer.entries(~D[2025-07-22])
#TodoGenServer.update_entry(1, fn e -> %{e | title: "Aggiornato"} end)
#TodoGenServer.delete_entry(1)
